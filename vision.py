        ...
        for color, (lower, upper) in self.color_ranges.items():
            ...
            if circles is not None:
                ...
                if len(self.circle_centers_queue[color]) >= 6:
                    ...
                    res = f"X {color_code} {((relative_x // 10 * 10) // 10)} {-((relative_y // 10 * 10) // 10)+6} Y"    #给单片机发送的信息
                    print(f"Generated data: {res}")

                    try:
                        # 通过串口发送数据
                        ser.write(res.encode("utf-8"))      #发送
...
def data_processor(rx_queue, tx_queue):
    ...
    cap = cv2.VideoCapture(9)   #每次联网后使用: 'ls /dev/video*' 查看摄像头设备所属 ，可能为 8 可能为 9
